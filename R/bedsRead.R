...
get_beds <- function(bedpaths, names=NULL, pattern='.bed'){
  ...
  for(i in c(1:length(bedpaths))){
    ...
    name <- ifelse(is.null(names), gsub(".bed","",tail(strsplit(current_path,"/")[[1]],1)), names[i])
...
get_singlebed <- function(bedpaths, sort=T, pattern='.bed'){
  bedpaths <- parse_bedpath(bedpaths, pattern=pattern)
  for(i in c(1:length(bedpaths))){
    if(i==1){
      df <- read_bed(bedpaths[i])[,1:3]
    }else{
      df <- rbind(df, read_bed(bedpaths[i])[,1:3])
    }
  }
