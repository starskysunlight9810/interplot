...
draw_venn <- function(input_df, bed_list, colors=c('#4E79A7FF','#F28E2BFF','#E15759FF','#76B7B2FF','#59A14FFF','#EDC948FF','#B07AA1FF'), opacity = 0.3, plotsize = 15){
...
draw_upset <- function(input_df, bed_list, decreasing=T, angles=45, text_scale=0.65, point_size=2.8, line_size=1){
...
draw_flower <- function(bedpaths, names=NULL, pattern='.bed', start=90, a=0.5, b=2, r=1.5, width=10, height=10, circle_text_cex=1,
  ...
  core_value <- dim(bed_merge(single_bed))[1]

  plot <- flower_plot(flower_lables, flower_values,labels=paste0('merged: ', core_value), start=start, a=a, b=b, r=r,
...
cal_pairwise <- function(bed_list, picture=T, method='pie', color="Blues",  tl_col="black"){
